...
export default function addFavorite() {
	...
	document.querySelector('#star').addEventListener('click', function(e) {
		...
		if (!value) {
			let keyName = city;
			let valueFavorite = `<li><a href="#city/${city}">${city}</a><button data-close="${city}" class="close btn"><img data-close="${city}" src="img/close.svg"></button></li>`;
			setStorage(keyName, valueFavorite)
				.then(_ => {
					getStorage(keyName)
						.then(data => {
