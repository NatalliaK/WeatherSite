export default function writeWeatherByCurrentLocation(data) {
	...
	info.innerHTML = `<p><time>${date}</time></p><p><span class="block-info__img-show-weather block-info__img-show-weather--${icon}"></span><span>${cloudCover}</span></p><p><span>${temp}&#176;C</span></p><p>Скорость ветра: ${speed} м/с</p>`;
}